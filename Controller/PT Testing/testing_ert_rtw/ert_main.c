...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) (testing_M->Timing.clockTick0+
    testing_M->Timing.clockTickH0* 4294967296.0);
  testing_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(testing_M->extModeInfo, (extmodeSimulationTime_T *)
    rteiGetPtrTFinalTicks(testing_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(testing_M, true);
    }
  }
